    ...
    // --- Curiosidades — Pins interactivos ---
    const curiosidadePins   = document.querySelectorAll('.curiosidade-pin');
    const curiosidadePopups = document.querySelectorAll('.curiosidade-popup');

    function fecharTodosPopups() {
        curiosidadePopups.forEach(p => p.classList.remove('visivel'));
        curiosidadePins.forEach(p => p.classList.remove('ativo'));
    }

    curiosidadePins.forEach(pin => {
        pin.addEventListener('click', function(e) {
            e.stopPropagation();
            const popup = document.getElementById(this.dataset.target);
            const eraVisivel = popup.classList.contains('visivel');
            fecharTodosPopups();
            if (!eraVisivel) {
    ...
    const secCuriosidades = document.getElementById('curiosidadesSection');
    if (secCuriosidades) {
        secCuriosidades.addEventListener('click', fecharTodosPopups);
    }

    // --- Utilização no Dia a Dia — Pins interactivos ---
    const utilizacaoPins   = document.querySelectorAll('.utilizacao-pin');
    const utilizacaoPopups = document.querySelectorAll('.utilizacao-popup');

    function fecharTodosUtilizacaoPopups() {
        utilizacaoPopups.forEach(p => p.classList.remove('visivel'));
        utilizacaoPins.forEach(p => p.classList.remove('ativo'));
    }

    utilizacaoPins.forEach(pin => {
        pin.addEventListener('click', function(e) {
            e.stopPropagation();
            const popup = document.getElementById(this.dataset.target);
            const eraVisivel = popup.classList.contains('visivel');
            fecharTodosUtilizacaoPopups();
            if (!eraVisivel) {
                popup.classList.add('visivel');
                this.classList.add('ativo');
            }
    ...
    document.querySelectorAll('#vantagensSection .popup-fechar').forEach(btn => {
        btn.addEventListener('click', function(e) {
            e.stopPropagation();
            fecharTodosUtilizacaoPopups();
        });
